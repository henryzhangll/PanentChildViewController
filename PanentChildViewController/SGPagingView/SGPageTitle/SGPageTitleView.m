...
- (void)layoutSubviews {
    ...
    if (lastBtn && lastBtn.tag >= _selectedIndex && _selectedIndex >= 0) {
...
- (UIView *)indicatorView {
    if (!_indicatorView) {
        ...
        _indicatorView.hidden = self.configure.indicatorHidden;
...
//        _bottomSeparator.frame = CGRectMake(bottomSeparatorW/2.0-13/2.0, bottomSeparatorY, 13, bottomSeparatorH);
...
//            [btn addTarget:self action:@selector(P_btn_action:) forControlEvents:(UIControlEventTouchUpInside)];
            ...
            UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(btn_action:)];
...
//
-(void)btn_action:(UITapGestureRecognizer *)tap{
    UIButton *btn =(UIButton*)tap.view;
    if ([self.delegatePageTitleView respondsToSelector:@selector(pageTitleView:btnSelectedIndex:)]) {
        [self.delegatePageTitleView pageTitleView:self btnSelectedIndex:btn.tag];
    }
    [self P_btn_action:btn];
}
...
- (void)P_changeIndicatorViewLocationWithButton:(UIButton *)button {
    [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
        if (self.configure.indicatorStyle == SGIndicatorStyleFixed) {
            ...
            self.indicatorView.SG_width = tempIndicatorWidth;//tempIndicatorWidth;
            self.indicatorView.SG_centerX = button.SG_centerX;
        }
    }];
}
